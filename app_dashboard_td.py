...
def detect_phase_cols(df: pd.DataFrame) -> list:
    ...
    for c in cand:
        ...
        if not name:
            continue
        s = df[c]
        # Preferir texto/categórico o pocas categorías
...
("Descargar Excel TD (fases)", data=out.getvalue(), file_name="td_tablas_fases.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
