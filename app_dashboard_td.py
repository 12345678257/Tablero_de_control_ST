...
def build_td_table(df: pd.DataFrame, phase_col: str, meses_order: list):
    # Usar nombres temporales para evitar colisiones al resetear índice / insert de groupers
...
fact = base[base['_FACTURADO_']=='Facturado'].groupby('_MES_', dropna=False)['_VALOR_'].sum().rename('Valor_Facturado')
nofact = base[base['_FACTURADO_']=='No Facturado'].groupby('_MES_', dropna=False)['_VALOR_'].sum().rename('Valor_No_Facturado')
kpi_mes = kpi_mes.merge(fact, left_on='Mes', right_index=True, how='left').merge(nofact, left_on='Mes', right_index=True, how='left').fillna(0)
...
with pd.ExcelWriter(out, engine="xlsxwriter") as writer:
    ...
    if phase_cols:
        for c in phase_cols:
            ...
            grp2 = grp.copy(); grp2.insert(0, 'Columna_Fase', c)
